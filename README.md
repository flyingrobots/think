...
- **Capture never blocks the agent.** Raw text is committed locally first. Derived graph work runs as follow-through and is abandoned after 6 seconds with a warning rather than holding the tool call open. Network backup is best-effort and never gates the local save.
