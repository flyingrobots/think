...
test('capture latency benchmark with --profile produces phase-level timing breakdown', () => {
  ...
  const expectedPhases = ['nodeStartup', 'repoEnsure', 'rawSave', 'finalization'];
...
test('capture latency profile phases sum to approximately the total end-to-end time', () => {
  ...
  // Phases should account for at least 80% of total time (some overhead is expected)
  assert.ok(
    phaseSum >= totalMedian * 0.8,
    `Phase sum (${phaseSum.toFixed(0)}ms) should be at least 80% of total (${totalMedian.toFixed(0)}ms).`
  );
});
